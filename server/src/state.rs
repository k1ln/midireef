...
pub struct AppState {
    ...
    /// Ausgewähltes Audio-Aufnahme-/Wiedergabe-Interface (Transport → RC).
    /// Persistiert als `<data_dir>/audio.json`, siehe `audio`.
    ...
    /// Laufende Audio-Wiedergabe (`audio.play.start`) — nur EINE gleichzeitig.
    pub audio_playback: Arc<Mutex<Option<crate::audio::ActivePlayback>>>,
