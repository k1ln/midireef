...
- **Touch-Ripple** (`ui/src/ui/ripple.ts`): ein `window`-`pointerdown`-Listener prüft `getComputedStyle(target).cursor === "pointer"` auf dem tatsächlich getroffenen DOM-Element (egal ob React-Button, -Tile, -Keyboard-Taste, …) und hängt einen Wassertropfen (weicher Kern + aufblühender Ring) in `.ripple-layer` — ein eigener `<body>`-Layer mit `pointer-events: none` ÜBER beiden Ebenen. Bewusst DOM statt Pixi: in der Hintergrund-Ebene lag der Tropfen hinter jeder deckenden Fläche (Knöpfe, Raster-Zellen, Popups) und war bei Hintergrund-Preset „off" gar nicht zu sehen. Animiert werden nur `transform`/`opacity` (Compositor-only), deshalb läuft er auch bei `data-motion="off"` weiter — der Schalter zielt auf die Dauer-Animationen, nicht auf Bedien-Rückmeldung.
...
- **WLAN-Access-Point:** Einstellungen → „Wi-Fi access point" lässt den Pi sein eigenes WLAN aufspannen (Name + Passwort in der UI, App selbst ohne Passwort). Der Server hält den Soll-Zustand in `data/network.json` (`net_ap.rs`), die Commands `network.getState` / `network.setAp` und das Event `network.state`. Geschaltet wird über das Einzige, das der Dienst als root darf: `deploy/bin/midireef-net` (locked-down, per `/etc/sudoers.d/midireef-net`), das eine AP-Verbindung `midireef-ap` in NetworkManager schreibt — `802-11-wireless.mode ap`, `ipv4.method shared` (DHCP + NAT + dnsmasq gratis). Clients landen in `10.42.0.0/24`, der Pi ist `10.42.0.1`, die UI also unter `http://10.42.0.1:8787`. Der Pi hat **ein** WLAN-Radio: AP an ⇒ kein WLAN-Uplink mehr (Ethernet-Uplink bleibt und wird an die Clients weitergereicht). Beim Serverstart re-appliziert `main.rs` den gespeicherten Zustand, damit der AP Reboots übersteht.
