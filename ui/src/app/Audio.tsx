//! Audio — eigener Vollbild-Screen (Peer von Dashboard/Sequencer/Library/
//! Settings, s. App.tsx), kein Popup mehr. Zwei Spalten: links die Aufnahmen
//! (der eigentliche Inhalt — abspielen, herunterladen, löschen), rechts fest
//! angedockt die Ein-/Ausgangs-Wahl (das ist in erster Linie ein PLAYER: der
//! Ausgang gehört näher an die Aufnahmen als der seltener gebrauchte Eingang,
//! deshalb "Output" oben in der rechten Spalte, "Input" unten).
//!
...
/** Ergebnis von `audio.probeInputs` für EINEN Eingang — s. `Audio`. */
...
// Feste Breite der rechten Einstellungs-Spalte — analog zu den anderen festen
// Andock-Breiten der App (DOCK_W/SETTINGS_W in Overview.tsx): Zielgerät ist
// ein einzelnes, großes Kiosk-Display, keine beliebig schmale Fläche.
const SETTINGS_COL_W = 460;
 ...
 *  loser Absätze, damit die Blöcke auf einen Blick auseinanderfallen statt
 *  als eine große Textwand zu wirken. */
...
export function Audio() {
  ...
  return (
    <div className="audio-panel" style={{ position: "fixed", top: TRANSPORT_H, left: 0, right: 0, bottom: 0, display: "flex", flexDirection: "column" }}>
      <div style={{ padding: "16px 16px 0" }}>
        <div className="popup-title">Audio</div>
        <div className="popup-subtitle">
          Choose an output on the right to play recordings through the Pi's own speakers/interface,
          then use ▶/🔊 on a recording below. Record new takes from the ⏺ button in the transport bar.
        </div>
        {error && <div style={{ color: "var(--pal-danger)", fontSize: 15, marginBottom: 14 }}>{error}</div>}
      </div>

      <div style={{ flex: 1, minHeight: 0, display: "flex", gap: 16, padding: 16, paddingTop: 8 }}>
        {/* ── Links: die Aufnahmen — der eigentliche Inhalt dieses Screens. ── */}
        ...
        {/* ── Rechts: Ein-/Ausgangs-Wahl, fest angedockt. ── */}
        <div style={{ flex: `0 0 ${SETTINGS_COL_W}px`, width: SETTINGS_COL_W, overflowY: "auto" }}>
          <AudioSection
            title="Output"
            action={
              <Button
          ...
          >
            <div className="popup-subtitle">
              Nothing audible when you press ▶/🔊? Tap “Test tone” — it plays a short beep straight
              out of the selected output, no recording involved. If you don't hear that either, the
              recording isn't the problem — check the output device, cabling, or volume.
            </div>
